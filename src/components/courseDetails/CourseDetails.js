...
import PDFrender from '../pdfFile/PDFrender';

const CourseDetails = () => {
    const courseData = useLoaderData();
    const { id, img, teacher, description, title, price, ratings, time, course } = courseData
    return (
        <div>
            <div className='mb-10'>
                <div className='bg-orange-300 p-5 my-2 text-center text-3xl font-semibold relative'>
                    ...
                    <PDFDownloadLink document={<PDFrender courseD={courseData} />} filename="FORM">
                        {({ loading }) => (loading ? <button>Loading Document...</button> : <button><FaDownload className='absolute right-5 top-1/4' /></button>)}
                    </PDFDownloadLink>
                </div>
                <div>
